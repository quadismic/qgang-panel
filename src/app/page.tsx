...
export default async function Headquarters(){
 ...
 return <AppShell right={false}><div className="commandHQ"><section className="commandHero"><div className="heroWords left">DAİMA<br/>DAHA<br/>İLERİ</div><div className="commandPortrait" aria-hidden="true"><img src="/brand/quad-command.png" alt=""/></div><div className="heroWords right">NİZAM<br/>GÜÇ<br/>BİRLİK</div><div className="commandTitle"><span>QG // KARARGÂH</span><h1>Q-GANG</h1><b>KARARGÂH</b><i/><p>TOPLULUKTAN DAHA FAZLASI<br/>BİR DÜZEN</p>{!user&&<Link href="/login">KİMLİĞİNİ DOĞRULA</Link>}</div></section><section className="commandModules">{cards.map(c=><Link href={c.href} className="commandPanel" key={c.href}><header><span>{c.icon}</span><h2>{c.title}</h2><small>{c.sub}</small></header><div><strong>{c.value}</strong><p>{c.text}</p></div><footer>İNCELE <b>→</b></footer></Link>)}</section><section className="commandMotto"><blockquote>“Düzen, özgürlüğün en güçlü hâlidir.”</blockquote><span>Q-GANG</span><div className="mottoPortrait" aria-hidden="true"><img src="/brand/quad-command.png" alt=""/></div><p>SİSTEM<br/>İNSANLARLA<br/>YAŞAR.</p></section><footer className="secureLine"><i/> QG // BAĞLANTI KURULDU <span>VERITAS IN TENEBRIS</span></footer></div></AppShell>
}