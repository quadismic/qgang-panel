...
import {brandTheme, enabledCouncilMembers} from "@/config/brand-theme";
...
export default async function Headquarters(){
 ...
 return <AppShell right={false}><div className="commandHQ"><section className="commandHero" style={{"--command-bg":`url(${brandTheme.world.master})`,"--command-pos":brandTheme.world.views.headquarters.position,"--command-size":brandTheme.world.views.headquarters.size} as CSSProperties}><div className="heroWords left">DAİMA<br/>DAHA<br/>İLERİ</div><div className="commandCouncil" aria-hidden="true">{enabledCouncilMembers.map((m:any)=><img key={m.id} className={`councilMember ${m.side}`} src={m.src} alt=""/>)}</div><div className="commandPortrait" aria-hidden="true"><img src={brandTheme.command.quad} alt=""/></div><div className="heroWords right">NİZAM<br/>GÜÇ<br/>BİRLİK</div><div className="commandTitle"><h1>Q-GANG</h1><b>KARARGÂH</b><i/><p>TOPLULUKTAN DAHA FAZLASI<br/>BİR DÜZEN</p>{!user&&<Link href="/login">KİMLİĞİNİ DOĞRULA</Link>}</div></section><section className="commandModules">{cards.map(c=><Link href={c.href} className="commandPanel" key={c.href}><header><span>{c.icon}</span><h2>{c.title}</h2></header><div><strong>{c.value}</strong><p>{c.text}</p></div><footer>İNCELE <b>→</b></footer></Link>)}</section><section className="commandMotto"><blockquote>“Düzen, özgürlüğün en güçlü hâlidir.”</blockquote><span>Q-GANG</span><div className="mottoArtwork" aria-hidden="true"><img src={brandTheme.seal} alt=""/></div><p>SİSTEM<br/>İNSANLARLA<br/>YAŞAR.</p></section><footer className="secureLine"><i/> BAĞLANTI KURULDU <span>VERITAS IN TENEBRIS</span></footer></div></AppShell>
}