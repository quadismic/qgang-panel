...
export default async function Members({searchParams}:{searchParams:Promise<{q?:string}>}){
 ...
 return <AppShell right={false}>
  <section className="registryHero"><span>Q-GANG // REGISTRY</span><h1>Topluluk Sicili</h1><p>Q-GANG kimlikleri, görevleri ve bağlı hesapları. Her isim düzenin yaşayan bir parçasıdır.</p>
  ...
  <section className="registryLedger"><header><span>QG // ÜYE KAYITLARI</span><b>{term?`“${term}” SONUÇLARI`:"TÜM KİMLİKLER"}</b></header>
   <div className="registryRows">{people.map((p:any)=>{const live=p.last_seen_at&&now-new Date(p.last_seen_at).getTime()<15*60*1000;return <Link href={"/u/"+p.handle} className="registryPerson" key={p.id}>
    ...
    <div className="registryIdentity"><small>QG // @{p.handle}</small><h2>{p.display_name}</h2><p>{p.bio||"Q-GANG üyesi"}</p></div>
